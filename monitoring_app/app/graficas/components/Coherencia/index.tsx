...
import { CoherenciaResponse } from '@/types/graphics/coherencia'
...
export default function Coherencia({nodos}:{nodos: InfoNodo[]}) {
  ...
  const  mapFrecuenciaCoherencia = (frecuencias?: number[], coherencia?: number[]): [number, number][] | [] => {
    if(frecuencias && coherencia){
      return frecuencias.map((f, i) => [f, coherencia[i]]);
    }else{
      return []
    }
  }
  ...
  useEffect(() => {
    if(nodo.nodo_1 && nodo.nodo_2 && variables.variable_1 && variables.variable_2){
      ...
      customAxios
        ...
        .then((response) => {
          const data = mapFrecuenciaCoherencia(response.data?.frecuencias, response.data?.coherencia)
          setDatos(data);
          setLoading(false);
        })
        .catch((e) => {
  ...
  const options: ApexCharts.ApexOptions = {
    ...
    title: {
      text: `Coherencia espectral entre ${variables.variable_1.toUpperCase()} de ${nodo.nodo_1?.nombre} vs ${variables.variable_2.toUpperCase()} de ${nodo.nodo_2?.nombre}`,
      align: 'center',
    },
    xaxis: {
      title: {
  ...
  return (
    <section className="w-full bg-white rounded-xl shadow-md">
      ...
      >
        <h2 className="font-bold text-2xl text-blue">Coherencia</h2>
