...
import { CorrelacionEntrePuntosResponse } from '@/types/graphics/correlacionentrepuntos'
...
export default function CorrelacionEntrePuntos({nodos}:{nodos: InfoNodo[]}) {

  const [isOpen, setIsOpen] = useState(false)
  const [loading, setLoading] = useState(false);
  const [datos, setDatos] = useState<[number, number][] | null>(null)
  const [correlacion, setCorrelacion] = useState<number>(0);
  ...
  useEffect(() => {
    if(nodo.nodo_1 && nodo.nodo_2 && variables.variable_1 && variables.variable_2){
      setLoading(true);
      customAxios
        .post<CorrelacionEntrePuntosResponse>('api/v1/analisis/correlacionpuntos', {
        ...
        .then((response) => {
          const data = mapFrecuenciaCoherencia(response.data?.puntos[variables.variable_1], response.data?.puntos[variables.variable_2])
          setDatos(data);
          setCorrelacion(response.data.correlacion);
          setLoading(false);
        })
        .catch((e) => {
          setDatos(null);
  ...
  const options: ApexCharts.ApexOptions = {
    ...
    title: {
      text: `Correlación entre ${variables.variable_1.toUpperCase()} de ${nodo.nodo_1?.nombre} vs ${variables.variable_2.toUpperCase()} de ${nodo.nodo_2?.nombre} (Correlación: ${correlacion})`,
      align: 'center',
    },
    xaxis: {
      type: 'numeric',
      title: {
        text: variables.variable_1,
      },
      decimalsInFloat: 4,
      tickAmount: 8,
      labels: {
  ...
  return (
    <section className="w-full bg-white rounded-xl shadow-md">
      ...
      >
        <h2 className="font-bold text-2xl text-blue">Correlación entre puntos</h2>
